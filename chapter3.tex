...
If a node goes offline it has to be detected by its peers. Therefor, an acknollege message (short: ACK) is send back on receieving a message. If such an ACK is not received within a timeout, the message can be resent up to a selectable number of times. If no ACK is received by then, the peer is no longer considered the be connected.
