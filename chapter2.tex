...
For measuring and comparing routing algorythms several metrices exist. Not all of them a of practical use if it comes to peer-to-peer and darknet networks. A simple first aproach  is the duration a packet takes to reach its destination, often refered to as the ping to the destination. But since this depends on many factors that can vary over time, it is not quite comparable and thus unusable for comparing routing algorythms.
A more stable and thus comparable metric is the path lenght. It is the amount of hops a packet has to be forwarded on until it reaches its destination. The packet length is a common
